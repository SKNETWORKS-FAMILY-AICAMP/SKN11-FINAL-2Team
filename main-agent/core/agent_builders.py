...
def _collect_all_data(place_response, profile, location_request, session_info, llm):
    ...
    if getattr(session_profile, "car_owned", None) is not None:
        car_status = "있음" if session_profile.car_owned else "없음"
        profile_info.append(f"차량: {car_status}")
    if getattr(session_profile, "free_description", None):
        profile_info.append(f"데이트 취향: {session_profile.free_description}")
    if getattr(session_profile, "general_preferences", None):
        preferences_text = ", ".join(session_profile.general_preferences) if isinstance(session_profile.general_preferences, list) else str(session_profile.general_preferences)
        profile_info.append(f"추가 선호사항: {preferences_text}")
...
2. 사용자의 데이트 취향(free_description), 추가 선호사항(general_preferences), 차량 유무, 교통수단 정보를 적극 활용하세요
3. RAG 데이터베이스 검색에 최적화된 자연스러운 한국어 문장으로 생성하세요
4. 다음 요소들을 포함해서 4-5문장으로 구성하세요:
   - 사용자가 원하는 분위기/상황
   - 누구와 함께하는지 (관계)
   - 어떤 경험을 원하는지
   - 사용자의 개인적 취향과 선호사항

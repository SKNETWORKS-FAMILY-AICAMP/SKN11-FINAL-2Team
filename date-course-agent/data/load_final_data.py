# 실제 장소 데이터를 벡터 DB에 로드하는 스크립트 (재시작 기능 추가)
# - data/places 디렉토리의 데이터를 읽어서 임베딩 생성 후 Qdrant에 저장
# - 중단된 지점부터 이어서 할 수 있는 기능 추가
...
class VectorDBLoader:
    ...
    def __init__(self):
        ...
        # 🔥 재시작 기능 추가
        self.processed_ids_file = "processed_place_ids.json"
        self.processed_ids = self.load_processed_ids()
        
        logger.info(f"✅ 벡터 DB 로더 초기화 완료 - 데이터 경로: {self.places_data_path}")
        logger.info(f"📋 이미 처리된 place_id: {len(self.processed_ids)}개")
    
    def load_processed_ids(self) -> Set[str]:
        """🔥 이미 처리된 place_id들 로드 (벡터 DB에서 추출 포함)"""
        # 1. 먼저 파일에서 로드 시도
        if os.path.exists(self.processed_ids_file):
            try:
                with open(self.processed_ids_file, 'r', encoding='utf-8') as f:
                    processed_list = json.load(f)
                logger.info(f"📁 파일에서 처리된 ID 로드: {len(processed_list)}개")
                return set(processed_list)
            except Exception as e:
                logger.warning(f"⚠️ 처리된 ID 파일 로드 실패: {e}")
        
        # 2. 파일이 없으면 벡터 DB에서 기존 place_id들 추출
        logger.info("📋 처리된 ID 파일이 없음. 벡터 DB에서 기존 데이터 확인 중...")
        return self.extract_existing_place_ids_from_db()
    
    def extract_existing_place_ids_from_db(self) -> Set[str]:
        """🔥 벡터 DB에서 기존 place_id들 추출"""
        try:
            ...
            logger.info(f"🔍 벡터 DB에서 {existing_count}개 데이터의 place_id 추출 중...")
            
            # Qdrant에서 모든 points의 place_id 추출
            existing_place_ids = set()
            
            try:
                # scroll 방식으로 모든 포인트 조회 (큰 데이터셋 대응)
                offset = None
                batch_size = 1000  # 한 번에 가져올 개수
                
                while True:
                    # Qdrant scroll API 사용 (실제 구현은 qdrant_client에 따라 다를 수 있음)
                    try:
                        # 방법 1: qdrant_client에 scroll 메서드가 있다면
                        result = self.qdrant_client.client.scroll(
                            collection_name="date_course_places",
                            limit=batch_size,
                            offset=offset,
                            with_payload=True,
                            with_vectors=False  # 벡터는 필요없고 payload만
                        )
                        
                        points = result[0]  # points 리스트
                        next_offset = result[1]  # 다음 offset
                        ...
                        # 진행 상황 표시
                        if len(existing_place_ids) % 5000 == 0:
                            logger.info(f"   📊 추출 진행: {len(existing_place_ids)}개...")
                        
                    except AttributeError:
                        # scroll 메서드가 없는 경우 대안 방법
                        logger.warning("⚠️ scroll 메서드 미지원. 대안 방법 사용")
                        break
                
                if existing_place_ids:
                    logger.info(f"✅ 벡터 DB에서 place_id {len(existing_place_ids)}개 추출 완료")
                    
                    # 🔥 추출한 ID들을 파일로 저장 (다음번 빠른 로드용)
                    try:
                        with open(self.processed_ids_file, 'w', encoding='utf-8') as f:
                            json.dump(list(existing_place_ids), f, ensure_ascii=False, indent=2)
                        logger.info(f"💾 추출한 ID들을 파일로 저장: {self.processed_ids_file}")
                    except Exception as e:
                        logger.warning(f"⚠️ ID 파일 저장 실패: {e}")
                    
                    return existing_place_ids
                else:
                    logger.warning("⚠️ 벡터 DB에서 place_id를 추출할 수 없음")
                    return set()
                    
            except Exception as e:
                logger.error(f"❌ 벡터 DB에서 place_id 추출 실패: {e}")
                logger.info("💡 안전 모드: 기존 데이터는 유지하고 새로운 데이터만 추가합니다")
                return set()  # 실패 시 빈 세트 반환 (안전 모드)
    ...
    def save_processed_ids(self):
        ...
        try:
            ...
            logger.warning(f"⚠️ 처리된 ID 저장 실패: {e}")
    ...
    async def load_all_data(self):
        ...
        try:
            ...
            for category_file in self.category_files:
                ...
                loaded_count, skipped_count = await self.load_category_data(category_file, category_name)
                if loaded_count > 0 or skipped_count > 0:
                    successful_categories += 1
                    total_loaded += loaded_count
                    total_skipped += skipped_count
                    logger.info(f"✅ {category_name} 완료 - 새로 로드: {loaded_count}개, 스킵: {skipped_count}개")
                    # 중간 진행 상황 확인
                    temp_info = self.qdrant_client.get_collection_info()
                    print(f"📈 진행 상황: {temp_info['points_count']}개 저장됨")
                else:
            ...
            logger.info(f"   새로 로드: {total_loaded}개")
            logger.info(f"   스킵됨: {total_skipped}개")
            logger.info(f"   소요 시간: {duration:.1f}초")
            
            # 컬렉션 정보 확인
            collection_info = self.qdrant_client.get_collection_info()
            logger.info(f"📊 최종 컬렉션 정보: {collection_info}")
            print(f"🎯 현재 저장된 벡터 수: {collection_info['points_count']}개")
            
            if total_loaded == 0 and total_skipped == 0:
    ...
    async def load_category_data(self, category_file: str, category_name: str) -> tuple[int, int]:
        """특정 카테고리 데이터 로드 (return: 새로 로드된 개수, 스킵된 개수)"""
        try:
            ...
            # 🔥 데이터 검증 및 필터링 (중복 체크 추가)
            valid_data, skipped_count = self.validate_and_filter_data(raw_data, category_name)
            logger.info(f"✅ 새로 처리할 데이터 - {len(valid_data)}개 항목, 스킵: {skipped_count}개")
            
            if not valid_data:
                logger.info(f"⏭️ {category_name}에 새로 처리할 데이터 없음")
            ...
            for i in range(0, len(valid_data), self.batch_size):
                ...
                # 실시간 points 확인 (터미널 출력)
                try:
                    current_info = self.qdrant_client.get_collection_info()
                    print(f"🎯 실시간 저장 상태: {current_info['points_count']}개 ✅")
                except Exception as e:
                    print(f"⚠️ points 확인 실패: {e}")
                
                # 배치 간 대기 (API 레이트 리밋 방지)
                if i + self.batch_size < len(valid_data):
                    await asyncio.sleep(2)
            
            return total_processed, skipped_count
            
        except Exception as e:
            logger.error(f"❌ {category_file} 처리 실패: {e}")
            return 0, 0
    
    def validate_and_filter_data(self, raw_data: List[Dict], category_name: str) -> tuple[List[Dict], int]:
        """🔥 데이터 검증 및 필터링 (중복 체크 추가)"""
        valid_data = []
        skipped_count = 0
        ...
        for item in raw_data:
            try:
                ...
                # 🔥 중복 체크 (이미 처리된 place_id인지 확인)
                place_id = item['place_id'].strip()
                if place_id in self.processed_ids:
                    skipped_count += 1
                    continue
                ...
                valid_item = {
                    'place_id': place_id,  # 실제 place_id 사용
        ...
        if skipped_count > 0:
            logger.info(f"   ⚠️ 스킵된 항목: {skipped_count}개 (중복 포함)")
        
        return valid_data, skipped_count
    ...
    async def process_batch(self, batch: List[Dict], category_name: str) -> int:
        ...
        try:
            ...
            # 🔥 처리된 place_id들을 processed_ids에 추가
            for item in batch:
                self.processed_ids.add(item['place_id'])
    ...
    def cleanup_on_completion(self):
        ...
        try:
            if os.path.exists(self.processed_ids_file):
                ...
                logger.info(f"🗑️ 완료 후 임시 파일 정리: {self.processed_ids_file}")
        except Exception as e:
...
async def main():
    """메인 실행 함수"""
    try:
        print("🚀 벡터 DB 로더 v2.0 (재시작 기능 포함)")
        print("="*60)
        
        # 🔥 현재 상태 확인
        temp_client = get_qdrant_client()
        current_info = temp_client.get_collection_info()
        current_count = current_info['points_count']
        
        print(f"📊 현재 벡터 DB 상태: {current_count:,}개 저장됨")
        
        if current_count > 0:
            print("✅ 기존 데이터가 발견되었습니다.")
            print("💡 이미 처리된 데이터는 자동으로 스킵하고 새로운 데이터만 처리합니다.")

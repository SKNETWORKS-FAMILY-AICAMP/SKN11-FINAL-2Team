...
class VectorDBLoader:
    ...
    def validate_and_filter_data(self, raw_data: List[Dict], category_name: str) -> List[Dict]:
        ...
        for item in raw_data:
            try:
                # 필수 필드 확인
                if not all(key in item for key in ['id', 'name', 'latitude', 'longitude', 'summary']):
                    skipped_count += 1
                    continue
                ...
                # 유효한 데이터로 변환
                valid_item = {
                    'place_id': f"{category_name}_{item['id']}",
        ...
        try:
            ...
            for item, embedding in zip(batch, embeddings):
                place_data = {
                    ...
                    'description': f"{item['description']} {item['summary']}".strip(),
